...
class FaceEmbedder:
    ...
    async def embed(self, image: Image.Image) -> list[float]:
        ...
        outputs = await asyncio.get_event_loop().run_in_executor(self.executor, self._predict, arr)
    ...
    def _predict(self, tensor: np.ndarray[Any, Any]) -> list[Any]:
        return cast("list[Any]", self.model.run(None, {self.input_name: tensor}))
