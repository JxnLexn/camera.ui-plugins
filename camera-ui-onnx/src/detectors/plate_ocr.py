...
class PlateOCR:
    ...
    async def recognize(self, plate_image: Image.Image) -> OcrResult | None:
        ...
        # CCT ONNX input is uint8 NHWC [1, 64, 128, 3] (0-255).
    ...
    def _predict(self, input_array: np.ndarray[Any, Any]) -> list[Any]:
        return cast("list[Any]", self.model.run(None, {self.input_name: input_array}))

    @staticmethod
    def _decode_output(outputs: list[Any]) -> OcrResult | None:
        plate_logits = None
        for val in outputs:
            arr = np.array(val)
