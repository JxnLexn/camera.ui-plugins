...
import { PamDiffMotionSensor } from './sensor.js';

import type { CameraDevice, DeviceStorage, LoggerService, PluginAPI } from '@camera.ui/sdk';

export default class PamDiff extends BasePlugin {
  private sensors = new Map<string, PamDiffMotionSensor>();
  ...
  private async addSensorToCamera(camera: CameraDevice): Promise<void> {
    const sensor = new PamDiffMotionSensor();
